    ...
    <form method="POST" action="{{ route('login') }}" class="space-y-5" x-data="{ loading: false, showPassword: false }" @submit="loading = true">
        ...
        <div class="flex items-center justify-between">
            <label for="remember_me" class="inline-flex items-center gap-2 cursor-pointer select-none">
                <input id="remember_me" type="checkbox" name="remember"
                       class="rounded-lg border-gray-300 text-[#3f8a5c] shadow-sm focus:ring-[#3f8a5c]/30 focus:ring-offset-0">
                <span class="text-sm text-gray-500">Remember me</span>
            </label>
            @if (Route::has('password.request'))
                <a href="{{ route('password.request') }}" class="text-sm text-[#1f5c3f] hover:text-[#173d2b] font-medium transition-colors">
                    Forgot password?
                </a>
            @endif
        </div>
        ...
        <div class="text-center space-y-1.5">
            <p class="text-sm text-gray-400">
                ...
                <a href="{{ route('register') }}" class="text-[#1f5c3f] hover:text-[#173d2b] font-medium transition-colors">Create one</a>
            </p>
            <p class="text-xs text-gray-300">
                Want to sell?
                <a href="{{ route('seller.register') }}" class="text-[#1f5c3f] hover:text-[#173d2b] font-medium transition-colors">Register as Seller</a>
                ·
                <a href="{{ route('seller.login') }}" class="text-[#1f5c3f] hover:text-[#173d2b] font-medium transition-colors">Seller Login</a>
            </p>
        </div>
    </form>
@endsection
