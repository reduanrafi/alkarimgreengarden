    ...
    <form method="POST" action="{{ route('seller.login') }}" class="space-y-5" x-data="{ loading: false, showPassword: false }" @submit="loading = true">
        ...
        <div class="flex items-center justify-between">
            ...
            @if (Route::has('password.request'))
                <a href="{{ route('password.request') }}" class="text-sm text-[#1f5c3f] hover:text-[#173d2b] font-medium transition-colors">Forgot password?</a>
            @endif
        </div>
        ...
        <p class="text-center text-sm text-gray-400">
            Not a seller?
            <a href="{{ route('login') }}" class="text-[#1f5c3f] hover:text-[#173d2b] font-medium transition-colors">Customer login</a>
        </p>
    </form>
@endsection
