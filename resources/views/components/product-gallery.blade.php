...
<div x-data="{ activeImage: '{{ $product->image ? asset('storage/' . $product->image) : '' }}', zoomIn: false, zoomPos: { x: 50, y: 50 } }" class="sticky top-24">
    ...
    @if(count($images) > 1)
        <div class="flex gap-2 mt-3 overflow-x-auto pb-2" x-on:mouseleave="zoomIn = false">
            @foreach($images as $idx => $img)
                <button @click="activeImage = '{{ asset('storage/' . $img) }}'; zoomIn = false"
                        class="shrink-0 w-16 h-16 sm:w-20 sm:h-20 rounded-xl border-2 overflow-hidden transition-all duration-200"
