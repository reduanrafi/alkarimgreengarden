...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <form method="GET" action="{{ route('search') }}" class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 sm:p-8 mb-8" data-ajax>
        <div class="relative mb-6">
            ...
            <input type="text" name="q" value="{{ request('q') }}" placeholder="Search by name, description, light needs, color..."
                   class="w-full pl-12 pr-4 py-3.5 rounded-xl border border-gray-200 text-sm focus:ring-2 focus:ring-[#3f8a5c]/20 focus:border-[#1f5c3f] outline-none bg-gray-50 transition">
        </div>

        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <div>
                <label class="block text-xs font-medium text-gray-500 mb-1.5">Category</label>
                <select name="category" class="w-full rounded-xl border border-gray-200 px-4 py-2.5 text-sm focus:ring-2 focus:ring-[#3f8a5c]/20 focus:border-[#1f5c3f] outline-none transition">
            ...
            <div>
                <label class="block text-xs font-medium text-gray-500 mb-1.5">Light</label>
                <select name="fabric" class="w-full rounded-xl border border-gray-200 px-4 py-2.5 text-sm focus:ring-2 focus:ring-[#3f8a5c]/20 focus:border-[#1f5c3f] outline-none transition">
            ...
            <div>
                ...
                <select name="color" class="w-full rounded-xl border border-gray-200 px-4 py-2.5 text-sm focus:ring-2 focus:ring-[#3f8a5c]/20 focus:border-[#1f5c3f] outline-none transition">
            ...
            <div>
                ...
                <select name="sort" class="w-full rounded-xl border border-gray-200 px-4 py-2.5 text-sm focus:ring-2 focus:ring-[#3f8a5c]/20 focus:border-[#1f5c3f] outline-none transition">
        ...
        <div class="flex flex-wrap items-center gap-3 mt-5 pt-5 border-t border-gray-100">
            <div class="flex items-center gap-2">
                ...
                <input type="number" name="min_price" value="{{ request('min_price') }}" placeholder="Min" class="w-20 sm:w-24 rounded-xl border border-gray-200 px-3 py-2 text-sm focus:ring-2 focus:ring-[#3f8a5c]/20 focus:border-[#1f5c3f] outline-none transition">
                <span class="text-gray-300">â€”</span>
                <input type="number" name="max_price" value="{{ request('max_price') }}" placeholder="Max" class="w-20 sm:w-24 rounded-xl border border-gray-200 px-3 py-2 text-sm focus:ring-2 focus:ring-[#3f8a5c]/20 focus:border-[#1f5c3f] outline-none transition">
            </div>
            <div class="flex items-center gap-2 ml-auto">
                <button type="submit" class="px-6 py-2.5 bg-[#1f5c3f] text-white text-sm font-semibold rounded-xl hover:bg-[#173d2b] transition shadow-sm flex items-center gap-2">
    ...
    @if(request()->anyFilled(['q','category','fabric','color','min_price','max_price']))
        <div class="flex items-center justify-between mb-6">
            <p class="text-sm text-gray-500">
                ...
                @if(request('q'))
                    for "<span class="text-[#1f5c3f] font-medium">{{ request('q') }}</span>"
                @endif
            </p>
        </div>
    @endif
    ...
    @if(request()->anyFilled(['q','category','fabric','color','min_price','max_price']))
        <div class="flex flex-wrap items-center gap-2 mb-6">
            @if(request('q'))
                <span class="inline-flex items-center gap-1 px-3 py-1 bg-[#e4efe4] text-[#1f5c3f] text-xs rounded-full border border-[#d5e6d5]">
                    "{{ request('q') }}"
                    <a href="{{ request()->fullUrlWithQuery(['q' => null]) }}" class="hover:text-[#173d2b] ml-1">&times;</a>
                </span>
            @endif
            @if(request('category'))
            ...
            @if(request('color'))
                <span class="inline-flex items-center gap-1 px-3 py-1 bg-[#e4efe4] text-[#1f5c3f] text-xs rounded-full border border-[#d5e6d5]">
                    {{ request('color') }}
                    <a href="{{ request()->fullUrlWithQuery(['color' => null]) }}" class="hover:text-[#173d2b] ml-1">&times;</a>
                </span>
            @endif
